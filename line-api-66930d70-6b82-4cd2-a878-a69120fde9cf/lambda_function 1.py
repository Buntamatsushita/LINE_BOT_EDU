...
user_list = {"aaaa":{"main_subject":"0","subject":"0","question":"0","answer":"0"}}
...
def lambda_handler(event, context):
    ...
    def handle_follow(event):
        ...
        account = {"user_ID":"0","main_subject":"0","subject":"0","question":"0","answer":"0"}
        user_list[name] = account
        user_ID = 
        user_list[name]["user_ID"] = 
    ...
    def handle_message(event):
        ...
        if event.message.text == "ヘルプ":
            ...
            FlexSendMessage(alt_text='教科を選択',
            ...
            flex_message_json_dict = json.loads(flex_message_json_string)
            line_bot_api.reply_message(event.reply_token,
            FlexSendMessage(alt_text='国語の科目を選択',
                # contentsパラメタに, dict型の値を渡す
                contents=flex_message_json_dict
            ))
            ...
            flex_message_json_dict = json.loads(flex_message_json_string)
            line_bot_api.reply_message(event.reply_token,
            FlexSendMessage(alt_text='数学の科目を選択',
                # contentsパラメタに, dict型の値を渡す
                contents=flex_message_json_dict
            ))
            ...
            flex_message_json_dict = json.loads(flex_message_json_string)
            line_bot_api.reply_message(event.reply_token,
            FlexSendMessage(alt_text='理科の科目を選択',contents=flex_message_json_dict))
            ...
            flex_message_json_dict = json.loads(flex_message_json_string)
            line_bot_api.reply_message(event.reply_token,
            FlexSendMessage(alt_text='地歴公民の科目を選択',contents=flex_message_json_dict))
            ...
            flex_message_json_dict = json.loads(flex_message_json_string)
            line_bot_api.reply_message(event.reply_token,
            FlexSendMessage(alt_text='英語の科目を選択',contents=flex_message_json_dict))
            ...
            fp.close() 
            s3.meta.client.upload_file("/tmp/" + file, 'line-api', "/tmp/" + file)
            os.remove("/tmp/" + file)
    ...
    #S3にJsonファイルが追加されたら(←トリガー)、jsonファイルのanswer="0"出ない限りuser_IDを呼び出してsend
